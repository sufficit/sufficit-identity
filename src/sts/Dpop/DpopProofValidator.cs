...
public sealed class DpopProofValidator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // iat: issued-at. MUST be present and recent (the proof is short-lived;
        // we bound the window via ValidateLifetime above, but also require the
        // claim to exist).
        if (!jwt.TryGetPayloadValue("iat", out long _))
        {
    ...
    private bool IsReplayedJti(string jti)
    {
        ...
        // Lazy eviction of expired entries keeps the cache from growing unbounded.
