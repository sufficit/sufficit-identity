...
public sealed partial class DocumentationContractTests
{
    ...
    /// <summary>
    /// Files in docs/activities/ use a YYYYMMDDHHmm-name.md timestamp format
    /// instead of the PREFIX-NAME.md convention used elsewhere.
    /// </summary>
    private static readonly Regex ActivityFilePattern =
        new(@"^\d{12}-[a-z0-9-]+\.md$", RegexOptions.Compiled);
    ...
    public void Documentation_names_express_their_purpose()
    {
        ...
        foreach (var path in markdownFiles)
        {
            ...
            // docs/activities/ uses YYYYMMDDHHmm-name.md timestamp format.
            var normalizedPath = path.Replace('\\', '/');
            if (normalizedPath.Contains("/activities/", StringComparison.Ordinal))
            {
                Assert.Matches(ActivityFilePattern, name);
                continue;
            }
