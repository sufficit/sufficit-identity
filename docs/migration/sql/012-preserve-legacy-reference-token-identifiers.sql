-- Preserve the identifiers and presentation metadata needed by account-
-- management APIs after the Duende -> OpenIddict cutover. These rows are
-- tombstones, never credentials: no payload, serialized grant data, session
-- identifier or usable token is copied, and every row is revoked.
--
